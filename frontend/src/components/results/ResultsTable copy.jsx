...
export default class ResultsTable extends React.PureComponent {
    ...
    update(completed) {
        let lastCount=this.results.items.length;
       ...
       // this.table.current.recomputeGridSize(0,lastCount)
                ...
                <AutoSizer>
                    {({width, height}) => {
                        return <VirtualList
                            ...
                            columnCount={1}
                            height={height}
                            renderItem={this._cellRenderer}
                            overscanCount={overscanRowCount}
                            itemCount={rowCount === 0 ? 2 : rowCount + 1}
        ...
        return <div key={index+"-"} style={rowStyle}>
            {
                (this.state.rowCount === 0 && rowIndex === 1) ?
                    ...
                    this.visibleColumns.map((column) => {
                        ...
                        return fn({ column, value, cellStyle});
                    })
            }
        </div>
